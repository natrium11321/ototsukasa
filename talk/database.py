...
class Database:
    ...
    def fetch_music_randomly(self):
        return self.fetch(u'select url,name AS address from (SELECT * FROM musics AS m LEFT JOIN toilets AS t ON m.toilet_id = t.id LEFT JOIN location AS l ON t.pos_id = l.pos_id ) order by rand() limit 1')[0]
